// packages/docsoft_scribe_core/lib/src/medicalization/medicalization_service.dart
//
// Medicalization service adapted for Dart-only (no Flutter dependencies).
...
class LocalMedicalizationService implements MedicalizationService {
  LocalMedicalizationService({
    required MedicalizationGlossary glossary,
    LogSink? logSink,
  })  : _glossary = glossary,
  ...
  Future<void> _ensureMappingsLoaded() async {
    ...
    _sortedMappings = fullMappings.entries
        .map(
          (e) => _SortedMapping(
            ...
            priority: MappingPriority.forCategory(e.value.category),
    ...
    _logSink.info(
      ...
      '(clinical: ${_sortedMappings!.where((m) => m.priority == MappingPriority.clinical).length}, '
      'voice_transforms: ${_sortedMappings!.where((m) => m.priority == MappingPriority.voiceTransforms).length})',
    );
  }
  ...
  static final Set<String> _commonClinicalTerms = {
    // General symptoms
    'fiebre', 'tos', 'dolor', 'cefalea', 'mareo', 'náuseas', 'vómito',
    'vómitos',
